var PCA = function(){
    ...
    function svd(A){
        ...
        for (i=0; i < n; i++)
        {
            ...
            for (j=i; j < m; j++) 
                s += (u[j][i]*u[j][i]);
            if (s <= tolerance)="" {="" g="0.0;" }="" else="" f="u[i][i];" if="" (f="">= 0.0) g= -g;
                h= f*g-s
                u[i][i]=f-g;
                for (j=l; j < n; j++)
                {
            ...
            for (j=l; j < n; j++) 
                s= s + u[i][j]*u[i][j]
            if (s <= tolerance)="" {="" g="0.0;" }="" else="" f="u[i][i+1]" if="" (f="">= 0.0) g= -g
                h= f*g - s
                u[i][i+1] = f-g;
        ...
        for (k=n-1; k != -1; k+= -1)
        {
            ...
            {// test f splitting
                ...
                for (l=k; l != -1; l+= -1)
                {
                    if (Math.abs(e[l]) <= prec){="" test_convergence="true" break="" }="" if="" (math.abs(q[l-1])="" <="prec)" (!test_convergence){="" cancellation="" of="" e[l]="" l="">0
                    c= 0.0
                    s= 1.0
                    var l1= l-1
                    for (i =l; i<k+1; i++)="" {="" f="s*e[i]" e[i]="c*e[i]" if="" (math.abs(f)="" <="prec)" break="" g="q[i]" h="pythag(f,g)" q[i]="h" c="g/h" s="-f/h" for="" (j="0;" j="" m;="" j++)="" y="u[j][l1]" z="u[j][i]" u[j][l1]="y*c+(z*s)" u[j][i]="-y*s+(z*c)" }="" test="" convergence="" (l="=" k){="" (z<0.0)="" q[k]="" is="" made="" non-negative="" n;="" v[j][k]="-v[j][k]" out="" of="" iteration="" loop and="" move="" on="" to="" next="" k="" value="" console.assert(iteration="" itmax-1,="" 'error:="" no="" convergence.');="" shift="" from="" bottom="" 2x2="" minor="" x="q[l]" (f="" 0.0)="" else="" qr="" transformation="" (i="l+1;" i<="" k+1;="" e[i-1]="z" v[j][i-1]="x*c+z*s" v[j][i]="-x*s+z*c" q[i-1]="z" u[j][i-1]="y*c+z*s" e[l]="0.0" e[k]="f" vt="transpose(v)" return="" (u,q,vt)="" (q[i]="" prec)="" sort="" eigenvalues="" i++){="" writeln(q)="">= 0; j--){
                if (q[j] < q[i]){
                    // writeln(i,'-',j)
                    c = q[j]
                    q[j] = q[i]
