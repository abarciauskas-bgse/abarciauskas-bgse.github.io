...
var yposition = function(d) {
    ...
    if (d.original > 0 && d.value > 0) { ypos += 2 }
    return ypos
}
...
var barheight = function(d) {
  ht = Math.abs(transfer_multiply_yscale(d.value))
  return ht
}
    ...
    var lineData = function(d, update) {
      ...
      if (update) {
        if (d.weight * d.value < 0) {
            ...
            y_update = transfer_multiply_height/2 - transfer_multiply_yscale(d.index) - transfer_multiply_yscale(Math.abs(d.value)) + barheight(d)/2 + 2
        }
      }
      return [ 
        [ barwidth, y_pos],
...
var draw_addition_links = function(node_data, group, group_indx, update, shift) {
    ...
    var lineData = function(d, update, shift) {
      ...
      var y_update = y_pos - 1
      if (update == 'first') {
        y_update = yposition_addition(d, group_indx) + barheight(d)/2 - 1
      } else if (update == 'second') {
        console.log(shift_scaled)
        y_update = yposition_addition(d, group_indx) + barheight(d)/2 - shift_scaled - 1
      }
...
var height_with_padding = function(d) {
    ...
    var padding = -2
    return ((height + padding) > 0 ? (height + padding) : 0)      
}
